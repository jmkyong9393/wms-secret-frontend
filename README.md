...
### 4. 실시간 상태 전파 — 보장과 최적화를 나눈다
검수 요청은 202로 즉시 접수되고 진행·판정은 비동기로 돌아옵니다. 그 상태를 화면에
옮기는 경로를 두 계층으로 나눠 둔 것이 핵심입니다.

| 계층 | 역할 |
| --- | --- |
| 폴링·재조회 (TanStack Query) | **보장** — 늦어도 반드시 맞춰진다 |
| SSE (`shared/lib/realtimeEvents.ts`) | **최적화** — 빠르면 좋다 |

SSE가 죽어도 폴링이 덮습니다. 그래서 놓친 이벤트를 되감는 장치(Last-Event-ID
리플레이)를 일부러 두지 않았습니다 — 재연결 시 전체 재조회로 충분합니다.

연결은 **화면 수와 무관하게 전역에 하나**만 엽니다. 화면마다 열면 서버가 연결마다
Redis pubsub을 만들 뿐 아니라, **그 화면을 보고 있을 때만** 이벤트를 받게 됩니다.
이벤트 → 무효화할 쿼리 키 매핑은 `app/providers.tsx`가 소유합니다.

인증은 **same-origin 프록시를 경유한 쿠키**로 합니다. EventSource가 커스텀 헤더를
붙일 수 없기 때문이며, 쿠키를 쓸 수 없는 환경을 위한 1회성 티켓 경로도 서버에
준비되어 있습니다(현재 미사용).

**조용히 죽는 경로 세 가지를 막아두었습니다** (2026-08-26 실측).
세 경우 모두 화면은 멀쩡하고 콘솔에도 오류가 남지 않아 발견이 어렵습니다.

1. **응답이 압축되면 스트림이 버퍼에 갇힌다.** 서버가 `no-transform`으로 막습니다.
2. **오류 없이 끊긴다.** 서버 하트비트(25초)가 끊기는지 감시해 판별합니다.
3. **브라우저가 재연결을 포기한다.** 오류 시 상태와 무관하게 직접 백오프로 다시 엽니다.
...
- 슬라이스 **내부**는 린트가 검사하지 못합니다. 세그먼트(`ui/ model/ api/ lib/ config/`)
  규칙과 현재 이탈 목록은 [프론트엔드 개발 가이드](docs/FRONTEND_GUIDE.md)에 있습니다.
- `app/`에는 라우트 규약 파일과 전역 조립(`providers.tsx`) 외에 컴포넌트를 두지 않습니다.
